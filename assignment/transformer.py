...
# BERT Transformer
# =============================================================================
def bert_attention(q, k, v, mask = None, dropout = None):
...
class bert_MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, y=None, mask=None):
        ...
        scores = bert_attention(q, k, v, mask, self.dropout) # BS * HEAD * SEQ_LEN * EMBED_SIZE_P_HEAD
        scores = scores.transpose(1,2).contiguous().view(scores.shape[0], -1, self.out_dim) # BS * SEQ_LEN * EMBED_SIZE_L
        out = self.out(scores)  # BS * SEQ_LEN * EMBED_SIZE

        return out
    
class bert_FeedForward(nn.Module):
...
class bert_EncoderLayer(nn.Module):
    def __init__(self, n_heads, inner_transformer_size, inner_ff_size, dropout=0.1):
        super().__init__()
        self.mha = bert_MultiHeadAttention(n_heads, inner_transformer_size, dropout)
        self.ff = bert_FeedForward(inner_transformer_size, inner_ff_size, dropout)
...
class bert_Transformer(nn.Module):
    def __init__(self, n_code, n_heads, embed_size, inner_ff_size, n_embeddings, seq_len, dropout=.1):
        ...
        self.pe = bert_PositionalEmbedding(embed_size, seq_len)

        #backbone
        encoders = []
        for i in range(n_code):
            encoders += [bert_EncoderLayer(n_heads, embed_size, inner_ff_size, dropout)]
...
class bert_PositionalEmbedding(nn.Module):
...
# GPT Transformer
# =============================================================================
class gpt_AttentionHead(nn.Module):
    """
...
class gpt_TransformerBlock(nn.Module):
    """
    ...
    def __init__(self, num_heads, block_size, num_embed, dropout):
        ...
        self.sa = gpt_MultiHeadAttention(
            num_heads=num_heads,
            head_size=head_size,
            num_embed=num_embed,
            block_size=block_size,
        ...
        self.ffwd = gpt_FeedForward(num_embed=num_embed, dropout=dropout)
...
# 1. Create a class that inherits from nn.Module
class vit_TransformerEncoderBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.msa_block = vit_MultiheadSelfAttentionBlock(embedding_dim=embedding_dim,
                                                     num_heads=num_heads,
                                                     attn_dropout=attn_dropout)

        # 4. Create MLP block (equation 3)
        self.mlp_block =  vit_MLPBlock(embedding_dim=embedding_dim,
                                   mlp_size=mlp_size,
                                   dropout=mlp_dropout)
...
class ViT(nn.Module):
    ...
    def __init__(self,
        ...
        self.patch_embedding = vit_PatchEmbedding(in_channels=in_channels,
                                              patch_size=patch_size,
                                              embedding_dim=embedding_dim)

        # 9. Create Transformer Encoder blocks (we can stack Transformer Encoder blocks using nn.Sequential())
        # Note: The "*" means "all"
        self.transformer_encoder = nn.Sequential(*[vit_TransformerEncoderBlock(embedding_dim=embedding_dim,
                                                                            num_heads=num_heads,
                                                                            mlp_size=mlp_size,
